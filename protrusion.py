...
def get_PDB_coords(filename, actual_directory):
  if filename.endswith(".pdb")== False:
    print("this file is not in correct format (i.e. not PDB){}".format(filename))
  length_dic={}
  ppdb = PandasPdb().read_pdb(os.path.join(actual_directory,filename))
  ppdb=ppdb.df['ATOM']
...
def calc_all(actual_directory,feature_directory):
  list_df=[]
  for filename in os.listdir(actual_directory):
    max_pos,max_length,res=get_PDB_coords(filename,actual_directory)
    name=filename.replace(".pdb","")
    line=[name,max_pos,res,max_length]
    list_df+=[line]
  df=pd.DataFrame(list_df,columns=["ID","tip_pos","tip_res","protrusion"],index=None)
  print(df)
  df.to_csv(os.path.join(feature_directory,"protrusion.csv"),header=True,index=None)
  return df


if __name__ == '__main__':
  df=calc_all(sys.argv[1],sys.argv[2])
