...
axes[0].set_ylabel('RMSD Global AA (Å)')
sns.jointplot(x="identity", y="localCA",data=df1,  orient='v' , ax=0)
...
axes[0].set_ylabel('RMSD Global AA (Å)')
axes[0].plot(y, a,'o',markersize=5)
axes[0].text(-0.4,22, "Pearson Correlation coefficient:{}".format(round(r2[0],2)), fontsize=10, fontweight='bold')
axes[0].set_title("Sequence Similarity", fontsize=14, fontweight='bold')

plt.show()
fig.savefig("seqId_similarity_RMSD_violin.png") 