...
def get_protrusion(file):
 	protrusion_df=protrusion_lib.calc_protrusion(filename)
...
def merge(file, ID, list_of_features, upload_dir):
	...
	if "tip_res" in list_of_features:
		protrusion_df=get_protrusion(file, ID)
		final_df.append(protrusion_df[["tip"]])

	if "protrusion" in list_of_features:
		protrusion_df=get_protrusion(file, ID)
		final_df.append(protrusion_df[["protrusion"]])

	if "happiness_mean" in list_of_features:
		happiness_df=get_happiness_score(file)
		final_df.append(happiness_df[["Happiness_mean"]])

	if "nr_sad" in list_of_features:
		happiness_df=get_happiness_score(file)
		final_df.append(happiness_df[["Nr_sad"]])
